...
def if_statement():
    ...
    if scanner.lookahead() == Token.END:
        AST = If_AST(condition, then)
    elif scanner.lookahead() == Token.ELSE:
        scanner.consume(Token.ELSE)
        otherwise = statements()
        AST = If_Else_AST(condition, then, otherwise)
    else: # error
        scanner.consume(Token.END, Token.ELSE)
...
def boolean_expression():
    ...
    while scanner.lookahead() in [Token.OR]:
        ...
        tree = boolean_term()
        result = Boolean_Expression_AST(result, op, tree)
    return result
...
def boolean_factor():
    if scanner.lookahead() == Token.NOT:
        ...
        result = Boolean_Expression_AST(boolean_factor(), op, None)
    else:
        result = comparison()
    return result
